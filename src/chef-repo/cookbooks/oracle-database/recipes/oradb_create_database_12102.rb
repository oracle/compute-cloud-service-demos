# coding: utf-8
#
# Cookbook Name:: oracle-dataguard
# Recipe:: oradb_create_database_12102
#
...
execute "Execute DBCA" do
  command "./dbca -silent -createDatabase -templateName #{node['oracle']['templateName']} -gdbname #{node['oracle']['gdbname']} -sid #{node['oracle']['sid']} -responseFile NO_VALUE -characterSet #{node['oracle']['characterSet']} -memoryPercentage #{node['oracle']['memoryPercentage']} -emConfiguration #{node['oracle']['emConfiguration']} -datafileDestination #{node['oracle']['datafileDestination']} -recoveryAreaDestination #{node['oracle']['recoveryAreaDestination']} -redoLogFileSize 1000 -sysPassword #{node['oracle']['sys_passwd']}  -systemPassword #{node['oracle']['system_passwd']}"
  user node['oracle']['user']
  group node['oracle']['group']
  cwd "#{node['oracle']['oracle_home_12c']}/bin"
  action :run
end


#Remove default Redologs and add custom sized logs
bash "add_redo_logs" do
    user node['oracle']['user']
    group node['oracle']['group']
	cwd "#{node['oracle']['oracle_home_12c']}/bin"
    environment ({ 'ORACLE_HOME' => "#{node['oracle']['oracle_home_12c']}"})
	code <<-EOH2
...
# create Listener.ora
file "#{node['oracle']['oracle_home_12c']}/network/admin/listener.ora" do
  owner 'oracle'
  group 'oinstall'
  ...
  mode '0644'
  not_if { ::File.exists?("#{node['oracle']['oracle_home_12c']}/network/admin/listener.ora")}
end


# Stop Listener
execute "Stop Listener" do
  command "./lsnrctl stop LISTENER"
  user node['oracle']['user']
  group node['oracle']['group']
  cwd "#{node['oracle']['oracle_home_12c']}/bin"
  returns [0,1]
