...
# Recipe:: oradb_create_database_11204
#
...
execute "Execute DBCA" do
  ...
  group node['oracle']['group']
  cwd "#{node['oracle']['oracle_home_11g']}/bin"
  action :run
end
  ...
  bash "add_redo_logs" do
    ...
    group node['oracle']['group']
	cwd "#{node['oracle']['oracle_home_11g']}/bin"
    environment ({ 'ORACLE_HOME' => "#{node['oracle']['oracle_home_11g']}"})
	code <<-EOH2
...
# create Listener.ora
file "#{node['oracle']['oracle_home_11g']}/network/admin/listener.ora" do
  owner 'oracle'
  group 'oinstall'
  ...
  mode '0644'
  not_if { ::File.exists?("#{node['oracle']['oracle_home_11g']}/network/admin/listener.ora")}
end
