...
template "/tmp/wlscohgeneric_silent.xml" do
  source 'wlscohgeneric_silent.xml.erb'
...
remote_file "server103x_generic" do
...
execute "Install Weblogic" do
  command "#{node['oracle']['local_jvms']}/bin/java -jar #{node['oracle']['installers']}/#{node['oracle']['wls_generic_installer']} -mode=silent -silent_xml=/tmp/wlscohgeneric_silent.xml"
